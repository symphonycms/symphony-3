<?php
	...
	-------------------------------------------------------------------------*/
		...
		public function prepareTableValue(StdClass $data = null, DOMElement $link = null) {
			...
			$data = (object)array(
				'value'	=> __($template, array($data->length))
			);
	...
	-------------------------------------------------------------------------*/
		...
		public function appendFormattedElement(&$wrapper, $data, $mode = null) {
			...
			$recover = $document->createElement('recover');
			$recover->setValue($driver->createToken($data->code, 'recover'));
			$result->appendChild($recover);

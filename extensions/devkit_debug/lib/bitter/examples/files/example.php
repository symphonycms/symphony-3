...
/*----------------------------------------------------------------------------*/
	...
	/*------------------------------------------------------------------------*/
		...
		public function __construct($language, $format = 'wrap-lines', $cache = false) {
			$this->cache = $cache;
			...
			if (is_readable($this->format_file)) {
				$class = include_once($this->format_file);
	...
	class BitterRule {
		...
		protected function getRule($context) {
			...
			foreach ($this->rules as $id) {
				...
				// No point trying any other rules:
				if ($position == 0) break;
			}
			
			//var_dump(Bitter::encode($position));
